...
function changeArrow(graphType){
...
graphType.onchange = () => {
    changeArrow(graphType);
    if (graphType.checked) {
        // classList just accesses the class of given element
        // 
...
function loadWelcomeConsoleText() {
    ...
    addTextToConsole("Built with ❤️ by Yash");
...
graphType.dispatchEvent(new Event("change")); // Triggering onchange event manually for first time 
...
function resetGraphData() {
    graph.changeData(data); // This modifies the graph actually
...
function buildGraphs(inputEdges) {
    ...
    inputEdges.shift(); // removing first element of the inputEdges array i.e 
                        // array containing number of nodes and edges
    v = nNodes;
    e = nEdges;
    visited = new Array(v).fill(false); // visited array created
    edgeList = inputEdges; // edge list created
    let dg = {}, udg = {};//To store directed and undirected graph in global variables
    let inputData = {nodes: [], edges: []};//inputData is to store data in global variable data
    let dm = [];// To store directed matrix in global variable
    let udm = [];// To store undirected matrix in global variable
    ...
    for (let i = 0; i < e; i++) {
        ...
        if (!(src in dg))//src in dg means to return true if  source is present as key in dg object
            dg[src] = [];
    ...
    data = inputData; // Storing the data which is given input seperately
...
function addTextToConsole(text) {
    // insertAdjacentHTML is a dom property
    consoleElement.insertAdjacentHTML("beforeEnd", `<p class="text-warning m-0"><span class="text-light">-></span> ${text}</p>`);
    //This line ensures that the console element scrolls to its bottom 
    //after adding the new message, making the latest message visible:
...
dfsButton.onclick = async () => {
    clearConsole();// Just clears the console
    resetGraph();//changes node colors to green,edges to black and visited as false
    let order = [];
...
async function kruskalsMST() {
    ...
    for (let i = 0; i < edgeListCopy.length; i++) {
        ...
        markEdgeVisiting(edgeIndex);
        await delay(delayValue);
        let verdict = doUnion(u, v); // returns false if both have same parent
...
async function bellmonFord(at = 0) {
    ...
    for (let j = 0; j < v; j++) {
        for (let i = 0; i < edgeList.length; i++) {
            markEdgeVisiting(i);
            markNodeVisiting(edgeList[i][0]);
            markNodeVisiting(edgeList[i][1]);
            await delay(delayValue);
            ...
            markEdgeVisited(i);
            markNodeVisited(edgeList[i][0]);
            markNodeVisited(edgeList[i][1]);
            await delay(delayValue);
        }
    }
